...
def run_pipeline(original_path: str, amendment_path: str) -> None:
    ...
    langfuse_handler = CallbackHandler(
        trace_name="contract-analysis-pipeline",
        metadata={
    ...
    try:
        print("🔬 LexTrace — Iniciando análisis...")

        # ──────────────────────────────────────────────
        # Paso 1: Extraer texto de ambas imágenes
        # ──────────────────────────────────────────────
        print("\n📸 Paso 1: Extrayendo texto de las imágenes...")

        print(f"   → Procesando contrato original: {original_path}")
        original_text = parse_contract_image(original_path, callbacks)
        original = ExtractedText(source="original", content=original_text)
        print(f"   ✅ Original: {len(original.content)} caracteres extraídos")

        print(f"   → Procesando adenda: {amendment_path}")
        amendment_text = parse_contract_image(amendment_path, callbacks)
        amendment = ExtractedText(source="amendment", content=amendment_text)
        print(f"   ✅ Adenda: {len(amendment.content)} caracteres extraídos")

        # ──────────────────────────────────────────────
        # Paso 2: Agente 1 — Cartógrafo 🗺️
        # ──────────────────────────────────────────────
        print("\n🗺️  Paso 2: Agente Cartógrafo — Mapeando correspondencias...")
        cartographer = ContextualizationAgent(callbacks=callbacks)
        mappings = cartographer.run(original.content, amendment.content)
        print(f"   ✅ {len(mappings)} secciones mapeadas")

        for m in mappings:
            status = (
                "🔄"
                if m.original_text and m.amended_text
                else ("➕" if not m.original_text else "➖")
            )
        ...
        detective = ExtractionAgent(callbacks=callbacks)
        ...
        print(
            "\n✅ Pipeline completado. Revisa Langfuse para el trace 'contract-analysis'."
        )
